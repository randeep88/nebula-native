...
const AlbumDetails = () => {
  ...
  const { addAlbum, removeAlbum, libraryItems, isAddingSong, isRemovingSong } =
    useLibrary();
  ...
  const { data: albumDetails, isPending } = useAlbumData(
  ...
  useEffect(() => {
    const getColor = async () => {
      const color = await getDominantColor(albumDetails?.image?.[2]?.url || "");
  ...
  const isAlbumInLibrary = libraryItems?.some(
        ...
        headerImage={
          <View className="relative">
            <Image
              source={{
                uri:
                  albumDetails?.image?.[2]?.url ?? "https://picsum.photos/400",
              }}
              style={{
                width: "100%",
                height: 300,
        ...
        <View className="p-3 flex-1">
          <View className="items-center justify-between flex-row">
            <Text className="text-white/80 font-semibold text-base">
              {formatNumbers(Number(albumDetails?.songs?.length))}{" "}
              {albumDetails?.songs?.length > 1 ? "songs" : "song"}
            </Text>
            ...
            <View className="items-center justify-between flex-row gap-4">
              {isAddingSong || isRemovingSong ? (
                ...
                <Pressable onPress={() => removeAlbum(albumId as string)}>
                ...
                <Pressable onPress={() => addAlbum(albumId as string)}>
                  <MaterialCommunityIcons
              ...
              {isSongInAlbum && isPlaying ? (
                ...
                <Pressable
                  onPress={() => handlePlaySong(albumDetails?.songs?.[0])}
                >
                  <MaterialCommunityIcons
                    name="play-circle"
                    color="#00CDAC"
